...
# Random is true for random frame, false for fixed frame
parser.add_argument("--random_frame", type=bool, default=False)
...
def play(env, vul_model, model, num_frames, neg_reward, random_frame):
    ...
    while not terminal:
        ...
        if step % num_frames == 0:
            if random_frame:
                frame = np.random.randint(num_frames)
            else:
...
if __name__ == "__main__":
    ...
    print("%s model with %d frames and random_frame = %s ends in %d steps." %
          (args.model,
           args.num_frames,
           args.random_frame,
           steps))
